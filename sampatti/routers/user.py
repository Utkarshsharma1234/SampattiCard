...
def contract_generation(request : schemas.Contract, db : Session = Depends(get_db)):
    ...
    static_pdf_path = os.path.join(os.getcwd(), 'contracts', f"{request.workerNumber}_employment_contract.pdf")

    return FileResponse(static_pdf_path, media_type='application/pdf', filename=f"{request.workerNumber}_employment_contract.pdf")
